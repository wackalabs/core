...
describe("SplitProxy via Factory", () => {
  describe("basic test", () => {
    ...
    describe("when there is a 50-50 allocation", () => {
      beforeEach(async () => {
        [funder, fakeWETH, account1, account2, transactionHandler] = await ethers.getSigners();
        ...
        const _deployTx = await proxyFactory.connect(funder).createSplit(rootHash);
        // Compute address.
        ...
        proxy = await (await ethers.getContractAt("SplitProxy", proxyAddress)).deployed();

        callableProxy = await (await ethers.getContractAt("Splitter", proxy.address)).deployed();
      });
      ...
      describe("and 1 ETH is deposited and the window is incremented", () => {
        ...
        describe("and one account claims on the first window", () => {
          ...
          beforeEach(async () => {
            ...
            claimTx = await callableProxy.connect(transactionHandler).claim(window, account, allocation, proof);
          ...
          it("it returns 1 ETH for balanceForWindow[0]", async () => {
            expect(await callableProxy.balanceForWindow(0)).to.eq(ethers.utils.parseEther("1").toString());
          });

          it("gets 0.5 ETH from scaleAmountByPercentage", async () => {
            expect(
              await callableProxy.scaleAmountByPercentage(allocation, ethers.utils.parseEther("1").toString()),
            ).to.eq(ethers.utils.parseEther("0.5").toString());
          });

          it("allows them to successfully claim 0.5 ETH", async () => {
            expect(amountClaimed.toString()).to.eq(ethers.utils.parseEther("0.5").toString());
          });

          it("costs 71894 gas", async () => {
            const { gasUsed } = await claimTx.wait();
            expect(gasUsed.toString()).to.eq("71894");
          });
          ...
          describe("and another 1 ETH is added, and the window is incremented", () => {
            ...
            describe("and the other account claims on the second window", () => {
              ...
              beforeEach(async () => {
                ...
                await callableProxy.connect(transactionHandler).claim(window, account, allocation, proof);
    ...
    for (let scenarioIndex = 0; scenarioIndex < scenarios.length; scenarioIndex++) {
      ...
      describe("#createSplit", () => {
        describe(`when the allocation is ${scaledPercentages.join("%, ")}%`, () => {
          ...
          it("costs 224096 gas to deploy the proxy", async () => {
          ...
          describe("when there is 100 ETH in the account and a window has been incremented", () => {
            ...
            for (let accountIndex = 0; accountIndex < allocationPercentages.length; accountIndex++) {
              describe(`and account ${accountIndex + 1} tries to claim ${firstDepositFirstWindow[
                ...
                it("successfully claims", async () => {
                  ...
                  const proof = tree.getProof(account, allocation);
                  const accountBalanceBefore = await waffle.provider.getBalance(account);
                  const tx = await callableProxy.claim(window, account, allocation, proof);
                  _gasUsed = (await tx.wait()).gasUsed;
                  const accountBalanceAfter = await waffle.provider.getBalance(account);
              ...
              describe(`and account ${accountIndex + 1} tries to claim with a higher allocation`, () => {
                it("reverts with 'Invalid proof'", async () => {
                  ...
                  const proof = tree.getProof(account, allocation);
                  await expect(callableProxy.claim(window, account, incorrectAllocation, proof)).revertedWith(
                    "Invalid proof",
                  );
                });
              });
            ...
            describe("and account 1 tries to claim twice in one window", () => {
              it("reverts on the second attempt", async () => {
                ...
                const { account, allocation } = ref;
                const proof = tree.getProof(account, allocation);
                await callableProxy.connect(transactionSigner).claim(window, account, allocation, proof);
                await expect(callableProxy.claim(window, account, allocation, proof)).revertedWith(
            ...
            for (let accountIndex = 0; accountIndex < allocationPercentages.length; accountIndex++) {
              describe(`and account ${accountIndex + 1} tries to claim twice in one window`, () => {
                it("reverts on the second attempt", async () => {
                  ...
                  await callableProxy.connect(transactionSigner).claim(window, account, allocation, proof);
                  await expect(
                    callableProxy.connect(transactionSigner).claim(window, account, allocation, proof),
              ...
              describe(`and account ${accountIndex + 1} tries to claim using claimForAllWindows`, () => {
                ...
                it("successfully claims", async () => {
                  ...
                  const proof = tree.getProof(account, allocation);
                  const accountBalanceBefore = await waffle.provider.getBalance(account);
                  _tx = await callableProxy.connect(transactionSigner).claimForAllWindows(account, allocation, proof);
                  const accountBalanceAfter = await waffle.provider.getBalance(account);
