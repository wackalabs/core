...
import { CrowdfundLogic } from "../../typechain/CrowdfundLogic";
...
const name = "Test Crowdfund";
const symbol = "TEST";
const CROWDFUND_STATUES = {
  ACTIVE: "ACTIVE",
};
...
describe("Crowdfund via Proxy from Factory", () => {
  ...
  before(async () => {
    [_deployerWallet, contributor, secondContributor, creatorWallet, funder, fundingRecipient] =
      ...
      describe("when the Crowdfund Factory is deployed", () => {
        ...
        describe("and a proxy is created through the factory", () => {
          ...
          beforeEach(async () => {
            ...
            proxy = await (await ethers.getContractAt("CrowdfundProxy", proxyAddress)).deployed();

            callableProxy = await (await ethers.getContractAt("CrowdfundLogic", proxyAddress)).deployed();
          });
          ...
          it("deletes parameters used during deployment", async () => {
            const { name, symbol, operator, fundingCap, operatorPercent } = await factory.parameters();
          ...
          it("uses 525931 gas", () => {
            expect(gasUsed.toString()).to.eq("525931");
          });
          ...
          describe("when a contributor attempts to contribute 2 ETH", () => {
            let originalBalance;
            const fundingAmount = "2000000000000000000"; // 2 ETH in wei
            let tx;
            let receipt;
            ...
            beforeEach(async () => {
              ...
              tx = await callableProxy.connect(contributor).contribute(contributor.address, fundingAmount, {
                value: fundingAmount,
              });
            ...
            it("uses 101419 gas", () => {
              expect(gasUsed.toString()).to.eq("101419");
            });
            ...
            it("mints tokens for the contributor equal to the amount of ETH given, multiplied by the token scale factor", async () => {
              ...
              const expectedBalance = BigNumber.from(fundingAmount).mul(TOKEN_SCALE);
            ...
            describe("when the contributor attempts to redeem 1.2 ETH worth their contributions", () => {
              ...
              it("uses 49924 gas", () => {
                expect(gasUsed.toString()).to.eq("49924");
              });
              ...
              describe("when another contributor adds 3.3 ETH", () => {
                let originalBalance;
                const fundingAmount = "3300000000000000000"; // 2 ETH in wei
                const tokenAmount = BigNumber.from(fundingAmount).mul(TOKEN_SCALE);
                ...
                it("uses 64419 gas", () => {
                  expect(gasUsed.toString()).to.eq("64419");
                });
                ...
                it("increases the contract's balance by 3.3 ETH", async () => {
                  ...
                  expect(contractBalance.toString()).to.eq(
                    BigNumber.from("800000000000000000").add(fundingAmount).toString(),
                  );
                });
                ...
                describe("when the contributor attempts to withdraw .2 ETH from their contributions", () => {
                  ...
                  describe("when the operator closes funding", () => {
                    ...
                    beforeEach(async () => {
                      originalFundsRecipientBalance = await provider.getBalance(fundingRecipient.address);
                      originalContractBalance = await provider.getBalance(callableProxy.address);
                      tx = await callableProxy.connect(creatorWallet).closeFunding();
                    ...
                    it("transfers the ETH to the funds recipient", async () => {
                      const updatedRecipientBalance = await provider.getBalance(fundingRecipient.address);
          ...
          describe("when a contributor attempts to contribute more than the funding cap", () => {
            const overfundingAmount = "20000000000000000000"; // 20 ETH
            let amountBefore;
            let amountAfter;
            let receipt;
            ...
            it("refunds the excess ETH while keeping the amount less the funding cap", async () => {
              ...
              const tx = await callableProxy.connect(contributor).contribute(contributor.address, overfundingAmount, {
              ...
              expect(balanceBefore.sub(balanceAfter).sub(receipt.gasUsed.mul(tx.gasPrice)).toString()).to.eq(
                fundingCap,
              );

              // Check that the user's token balance is correct.
              const tokenBalance = await callableProxy.balanceOf(contributor.address);
              expect(tokenBalance.toString()).to.eq(BigNumber.from(fundingCap).mul(TOKEN_SCALE).toString());
            });

            it("uses 108868 gas", () => {
              expect(receipt.gasUsed.toString()).to.eq("108868");
            });
            ...
            describe("when a contributor adds ETH once the funding cap is already reached", () => {
              it("reverts the transaction", async () => {
                ...
                await expect(txPromise).to.be.revertedWith("Funding cap already reached");
