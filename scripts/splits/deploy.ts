...
async function main() {
  ...
  const splitFactory = await SplitFactory.deploy(splitter.address, WETH_ADDRESS);
  ...
  if (!isLocal) {
    fs.writeFileSync(`${__dirname}/../../networks/splits/${networkName}.json`, JSON.stringify(info, null, 2));
  }
}
