...
contract CrowdfundLogic is CrowdfundStorage {
    ...
    function contribute(address payable backer, uint256 amount) external payable nonReentrant {
        require(status == Status.ACTIVE, "Crowdfund: Funding must be open");
        ...
        if (address(this).balance <= fundingCap) {
            ...
            require(startAmount < fundingCap, "Crowdfund: Funding cap already reached");
    ...
    function redeem(uint256 tokenAmount) external nonReentrant {
        ...
        // Check
        require(balanceOf[msg.sender] >= tokenAmount, "Crowdfund: Insufficient balance");
        // Effect
