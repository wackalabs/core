...
export default class BalanceTree {
  ...
  constructor(balances: { account: string; allocation: BigNumber }[]) {
    this.tree = new MerkleTree(
      balances.map(({ account, allocation }, _index) => {
        return BalanceTree.toNode(account, allocation);
      }),
    );
  }

  public static verifyProof(account: string, allocation: BigNumber, proof: Buffer[], root: Buffer): boolean {
  ...
  public static toNode(account: string, allocation: BigNumber): Buffer {
    return Buffer.from(utils.solidityKeccak256(["address", "uint256"], [account, allocation]).substr(2), "hex");
  }
