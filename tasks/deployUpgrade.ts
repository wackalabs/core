...
import { gasPrice, getSelectors, getSighashes } from "../scripts/helperFunctions";
...
export function convertFacetAndSelectorsToString(facets: FacetsAndAddSelectors[]): string {
  let outputString = "";

  facets.forEach(facet => {
    outputString = outputString.concat(
      `#${facet.facetName}$$$${facet.addSelectors.join("*")}$$$${facet.removeSelectors.join("*")}`,
    );
  });
...
export function convertStringToFacetAndSelectors(facets: string): FacetsAndAddSelectors[] {
...
task("deployUpgrade", "Deploys a Diamond Cut, given an address, facets and addSelectors, and removeSelectors")
  ...
  .addFlag("useMultisig", "Set to true if multisig should be used for deploying")
  ...
  .setAction(async (taskArgs: DeployUpgradeTaskArgs, hre: HardhatRuntimeEnvironment) => {
    const facets: string = taskArgs.facetsAndAddSelectors;
    const facetsAndAddSelectors: FacetsAndAddSelectors[] = convertStringToFacetAndSelectors(facets);
    ...
    let signer: Signer;
    const owner = await ((await hre.ethers.getContractAt("OwnershipFacet", diamondAddress)) as OwnershipFacet).owner();
    ...
    for (let index = 0; index < facetsAndAddSelectors.length; index++) {
      ...
      const factory = (await hre.ethers.getContractFactory(facet.facetName)) as ContractFactory;

      const deployedFacet: Contract = await factory.deploy({
        gasPrice: gasPrice,
      });
      await deployedFacet.deployed();
      console.log(`Deployed Facet Address for ${facet.facetName}:`, deployedFacet.address);
      ...
      for (const selector of newSelectors) {
        if (!existingFuncs.includes(selector)) {
          ...
          throw Error(`Selector ${selector} (${facet.addSelectors[index]}) not found`);
        }
      }

      let existingSelectors = getSelectors(deployedFacet);
      existingSelectors = existingSelectors.filter(selector => !newSelectors.includes(selector));
    ...
    //Execute the Cut
    const diamondCut = (await hre.ethers.getContractAt("IDiamondCut", diamondAddress, signer)) as IDiamondCut;

    if (testing) {
      console.log("Diamond cut");
      const tx: ContractTransaction = await diamondCut.diamondCut(cut, AddressZero, "0x", { gasLimit: 8000000 });
      ...
      if (useMultisig) {
        ...
        const tx: ContractTransaction = await diamondCut.diamondCut(cut, AddressZero, "0x", { gasLimit: 800000 });
